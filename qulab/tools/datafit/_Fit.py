...
class LRZ_Fit(BaseFit):
    ...
    def _fitfunc(self, t, f_center, kappa_int, kappa_ext):
        ...
        if self.fit_tag == 'real':
            y = 1 - 2 * kappa_ext / ((kappa_int + kappa_ext) *
                                     (1 + (2 * (t - f_center) /
                                           (kappa_int + kappa_ext))**2))
        elif self.fit_tag == 'imag':
            y = 2 * kappa_ext * (2 * (t - f_center) /
                                 (kappa_int + kappa_ext)) / (
                                     (kappa_int + kappa_ext) *
                                     (1 + (2 * (t - f_center) /
                                           (kappa_int + kappa_ext))**2))
        else:
            y = kappa_ext * kappa_int**2 / (4 *
                                            (t - f_center)**2 + kappa_int**2)
        return y
...
class S_Fit(BaseFit):
    ...
    def _fitfunc(self, t, A, alpha, delay, phi, Q_l, Q_c, f_c):
        ...
        if 'S11' in self.fit_tag:
            S = A * np.exp(1j * alpha) * np.exp(
                -2 * np.pi * 1j * t *
                delay) * (1 - (abs(Q_l) / abs(Q_c) * np.exp(1j * phi)) /
                          (1 + 2j * abs(Q_l) * (t / f_c - 1)))
        else:
            S = A * np.exp(1j * alpha) * np.exp(
                -2 * np.pi * 1j * t * delay) * (abs(Q_l) / abs(Q_c) * np.exp(
                    1j * phi)) / (1 + 2j * abs(Q_l) * (t / f_c - 1))
        return abs(S)
    ...
    def get_delay(self):
        ...
        if 'weak' in self.fit_tag:
            f_para = np.hstack(
                [f_para[:int(length / 5)], f_para[-int(length / 5):]])
            phase = np.unwrap(np.angle(S_para))
            phase = np.hstack(
                [phase[:int(length / 5)], phase[-int(length / 5):]])
            ...
            gradient, intercept, r_value, p_value, std_err = stats.linregress(
                abs(f_para), phase)
            self.delay = (gradient -
                          (-np.pi * 2. / abs(f_para[-1] - f_para[0]))) / (
                              np.pi * 2.)
        return self.delay

    ## 去除延时
    def cancel_delay(self):
        self.data[1] = self.data[1] * np.exp(2 * 1j * np.pi *
                                             (-self.delay) * self.data[0])
        phase = np.unwrap(np.angle(self.data[1]))
        self.data[1] = (np.cos(phase[0]) -
                        1j * np.sin(phase[0])) * self.data[1]
    ...
    def normalization(self):
        self.data[1] = self.data[1] / self._popt[0] * np.exp(
            -1j * self._popt_phase[1]) * np.exp(
                2 * np.pi * 1j * self.data[0] * self._popt_phase[2])
        return self.data
    ...
    def get_single_photon_limit(self, unit='dBm'):
        ...
        if unit == 'dBm':
            return 10. * np.log10(1000. / (4. * k_c / (2. * np.pi * hbar * fr *
                                                       (k_c + k_i)**2)))
        else:
    ...
    def plot(self, fig):
        t, y = self.data
        S_result = self.S_fit_result(t, 1, 0, 0, self._popt[3], self._popt[4],
                                     self._popt[5], self._popt[6])
        ...
        ax4.text(0.5,
                 0.9,
                 'f = ' + str(round(self.params[6] / 1e9, 6)) + 'GHz',
                 verticalalignment='center',
                 horizontalalignment='center')
        ax4.text(0.5,
                 0.75,
                 'Q_int = ' +
                 str(round(self.params[6] / self.params[5] / 1e3, 1)) + 'k',
                 verticalalignment='center',
                 horizontalalignment='center')
        ax4.text(0.5,
                 0.6,
                 'kappa_int = ' + str(round(self.params[5] / 1e3, 1)) + 'kHz',
                 verticalalignment='center',
                 horizontalalignment='center')
        ax4.text(0.5,
                 0.45,
                 'kappa_ext = ' + str(round(self.params[4] / 1e3, 1)) + 'kHz',
                 verticalalignment='center',
                 horizontalalignment='center')
        ax4.text(0.5,
                 0.3,
                 'delay = ' + str(round(self.params[2] * 1e9, 1)) + 'ns',
                 verticalalignment='center',
                 horizontalalignment='center')
        ax4.text(0.5,
                 0.15,
                 'mis-match = ' + str(round(self.params[3], 3)),
                 verticalalignment='center',
                 horizontalalignment='center')
    ...
    def error(self):
        ...
        return [
            self._error[0], self._error_phase[1], self._error_phase[2],
            self._error[3], kappa_ext_error, kappa_int_error, self._error[6]
        ]
    ...
    def params(self):
        ...
        return [
            self._popt[0] * self.A, self._popt_phase[1],
            self._popt_phase[2] + self.delay, self._popt[3],
            kappa_ext * np.exp(-1j * self._popt[3]), kappa_int, self._popt[6]
        ]
...
class Ramsey_Fit(BaseFit):
    ...
    def _fitfunc(self, t, A, B, C, Tphi, w):
        y = A * np.exp(-t / 2 / self._T1 -
                       np.square(t / Tphi)) * np.cos(w * t + C) + B
        return y
