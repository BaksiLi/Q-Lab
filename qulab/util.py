...
def get_unit_prefix(value: float) -> (str, float):
    """Retrieve number from prefix.
...
def dBmToW(p: float) -> float:
    """Convert dB to W.
    """
    return 10**(p / 10.0 - 3)


def WTodBm(p: float) -> float:
    """Convert W to dB.
    """
...
def get_probility(x, N, a=0.05):
    ...
    low, high = beta.ppf(0.5 * a, x + 1,
                         N - x + 1), beta.ppf(1 - 0.5 * a, x + 1, N - x + 1)
